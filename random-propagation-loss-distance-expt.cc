...
int main (int argc, char *argv[])
{
...

  {
	...
	Ptr<LogDistancePropagationLossModel> randomProp = CreateObject<LogDistancePropagationLossModel> ();
  randomProp->SetAttribute("Exponent", DoubleValue(2.5));
	randomProp->SetAttribute("RandomVariable", StringValue ("ns3::NormalRandomVariable[Mean=0|Variance=4]"));
        for (double distance = 50.0; distance <= 700.0; distance += 50.0)
	{
    ...
    plot.SetTitle ("LogDistancePropagationLossModel");
